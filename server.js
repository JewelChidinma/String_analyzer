...
const app = express();
app.use(express.json());
app.use(express.urlencoded({ extended: true })); // ✅ added for form-encoded requests
...
app.get('/strings/filter-by-natural-language', async (req, res) => {
  try {
    ...
    let parsed;
    try {
      parsed = parseNaturalLanguageQuery(q); // ✅ removed decodeURIComponent
    } catch (err) {
      if (err.code === 422) return res.status(422).json({ error: err.message });
      return res.status(400).json({ error: 'Unable to parse natural language query' });
    }
    ...
    results = results.filter(rec => {
      ...
      if (f.max_length !== undefined && p.length > f.max_length) return false;
      if (f.word_count !== undefined && p.word_count !== f.word_count) return false;
      if (f.contains_character !== undefined && !rec.value.toLowerCase().includes(f.contains_character.toLowerCase())) return false; // ✅ case-insensitive
      return true;
    });
...
app.get('/strings', async (req, res) => {
  try {
    ...
    results = results.filter(rec => {
      ...
      if (filters.max_length !== undefined && p.length > filters.max_length) return false;
      if (filters.word_count !== undefined && p.word_count !== filters.word_count) return false;
      if (filters.contains_character !== undefined && !rec.value.toLowerCase().includes(filters.contains_character.toLowerCase())) return false; // ✅ case-insensitive
      return true;
    });
